...
- **Twig template mode for the narration script.** A Bespoken field's new **Narration script source** setting can be switched from **Field handles** to **Twig template**, where you write the script as a Twig template rendered on the server against the entry (`entry`, or `object` with Craft's `{title}` shortcut syntax). Templates can loop over nested Matrix blocks at any depth, read related elements, add per-block-type phrasing, and use any Twig filter, and they are independent of how Matrix fields are displayed in the editor. Nested Matrix queries return only live blocks for the site, so disabled blocks are skipped automatically. HTML from CKEditor and Redactor fields in the output is converted to speech text exactly as it is for field handles, each line of output becomes a paragraph, and the result feeds the same preview, edit, and generate flow. The template is rendered against the entry's current draft: Bespoken asks Craft's element editor to save pending changes first, so the preview reflects unsaved edits. Syntax errors are caught when the field settings are saved, and a template that fails while rendering reports the error in the control panel. Field-handle mode is unchanged and remains the default.
- **Insert starter handles / starter template.** Both script sources come with an entry type picker and an insert button that generates a starting point from the chosen entry type's field layout. In field-handle mode, **Insert starter handles** writes the text field handles with each Matrix field's block fields in brackets, nested as deep as the content model goes (handles shared by several block types listed once), and reports the fields it left out in a notice. In template mode, **Insert starter template** generates a template: text fields one per line, a `{% for %}` loop for each Matrix field nested as deep as the content model goes, a `{% switch %}` on block type where a Matrix field has several, and a comment naming the fields that can't be narrated. Large nested content models no longer have to be typed out by hand.
...
- **Edit the narration script before generating.** The **Preview & edit script** dialog now has an **Edit script** button that turns the preview into an editor, so you can fix a name, cut a sentence, or add an intro without touching the entry. Edits change only the narration script: they are never saved to the entry, and while one is in force the field shows a status line ("Using an edited narration script. Your entry is unchanged.") with **Review script** and **Revert to entry text**, so it is always clear which text **Generate audio** and **Create Alias TTS project** will send. Both actions are also available inside the dialog. Closing the dialog keeps your edits (they survive a reload in the same browser tab); **Revert to entry text** discards them. Edited text is tidied the same way entry text is before sending — emoji and angle brackets removed, paragraphs given sentence-ending punctuation — with a note of how many unsupported characters were dropped, and a live character count (plus an ElevenLabs credit estimate) shows while editing. If the entry changes after a script was edited, nothing is sent silently: the field flags it, and generating opens the dialog to **Keep edited script** or **Revert to entry text** first. The preview also now shows exactly the text that is sent, with paragraph spacing normalized and no stray indents.
...
- Cleaned up several text artifacts that could surface as gaps or "swishing" noises in generated audio — most noticeably with the self-hosted Alias TTS service (Chatterbox), which is less forgiving of odd punctuation and whitespace than ElevenLabs:
  ...
  - **Paragraph breaks are normalized to exactly one blank line.** A bug could double an existing break into a triple newline, which surfaced as extra silence between paragraphs. Breaks are now kept as a single `\n\n` for both providers: the ElevenLabs path still sends a post that fits in one request with its blank lines intact, as before, and splits longer posts at paragraph boundaries first (joining paragraphs with a space inside each chunk); the Alias TTS service uses the blank line as a block seam to insert its longer paragraph pause instead of the short sentence gap.
