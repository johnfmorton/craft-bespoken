...
",
    ...
    'Script source instructions' => 'How the narration script is built. **Field handles** reads the listed fields from the entry editor. **Twig template** renders a template you write here against the entry, which can reach nested Matrix blocks, related elements, and anything else Twig can. **Twig template file** renders a template from your project\'s templates folder the same way, so only the path is stored in project config.',
    'Field handles' => 'Field handles',
    'Twig template' => 'Twig template',
    'Twig template file' => 'Twig template file',
    'Script template path' => 'Script template path',
    'Script template path instructions' => 'The path of a template in your project\'s templates folder, written like a section\'s entry template (e.g. `_narration/article` for `templates/_narration/article.twig`). It is rendered against the entry, available as `entry`, with the same output handling as an inline Twig template: HTML is converted to speech text and each line of output becomes a paragraph. To start from a generated template, switch to **Twig template**, click **Insert starter template**, and paste the result into your file.',
    'Enter a template path, or switch the script source to field handles.' => 'Enter a template path, or switch the script source to field handles.',
    'No template exists at "{path}" in the project\'s templates folder.' => 'No template exists at "{path}" in the project\'s templates folder.',
